...
export function LoginForm() {
    ...
    const errorMessage =
        errorParam === 'AccessDenied'
            ? 'This Google account is not authorised for this portal.'
            : errorParam
    ...
    return (
        <Card className="w-full max-w-md shadow-lg border-0 bg-white">
            <CardHeader className="text-center pb-6">
                ...
                <CardDescription>Sign in with your @iiitd.ac.in Google account</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
                {errorMessage && (
                    <p className="text-sm text-red-500 text-center bg-red-50 rounded-md py-2 px-3">
                        {errorMessage}
                    </p>
                )}
                <Button
