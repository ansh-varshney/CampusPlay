...
export default async function ActiveSessionsPage() {
    ...
    return (
        <div className="p-4 space-y-4">
            <h1 className="text-xl font-bold text-gray-900 dark:text-slate-100">Active Sessions</h1>
            <p className="text-sm text-gray-500 dark:text-slate-400 -mt-2">Sessions currently in progress</p>
            ...
            {activeBookings.length === 0 ? (
                <div className="p-8 text-center text-gray-400 dark:text-slate-500 border-2 border-dashed border-gray-200 dark:border-slate-800 rounded-xl">
                    <Timer className="w-8 h-8 mx-auto mb-2 text-gray-300 dark:text-slate-600" />
                ...
                <div className="space-y-3">
                    {activeBookings.map((booking: any) => (
                        <Link key={booking.id} href={`/manager/approvals/${booking.id}`}>
                            <Card className="hover:shadow-md transition-shadow cursor-pointer border-l-4 border-l-green-500 mb-3 bg-white dark:bg-slate-900 border-gray-200 dark:border-slate-800">
