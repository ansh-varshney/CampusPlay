...
import { Bell, CheckCheck, AlertTriangle, CheckCircle, Info, CalendarPlus } from 'lucide-react'
import { formatDistanceToNow } from 'date-fns'
import { useRouter } from 'next/navigation'

function notifIcon(type: string) {
    if (type === 'emergency_alert' || type === 'equipment_incident') {
        return <AlertTriangle className="w-5 h-5 text-red-500 shrink-0 mt-0.5" />
    }
    if (type === 'new_booking') {
        return <CalendarPlus className="w-5 h-5 text-blue-500 shrink-0 mt-0.5" />
    }
    if (type.includes('completed') || type.includes('active')) {
...
export function ManagerNotificationsClient({
    notifications,
}: {
                ...
                <section key={label} className="space-y-2">
                    ...
                    {items.map((n) => (
                        <div
                            ...
                            className={cn(
                                'p-4 rounded-xl border bg-white dark:bg-slate-900 cursor-pointer transition-colors',
                                n.is_read
                                    ? 'border-gray-100 dark:border-slate-800 opacity-70'
                                    : 'border-[#004d40]/20 dark:border-teal-500/30 shadow-sm hover:border-[#004d40]/40'
                            )}
                        >
