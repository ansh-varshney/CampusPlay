...
const VIOLATION_LABELS: Record<string, { label: string; emoji: string; color: string }> = {
    lost_equipment: {
        ...
        emoji: '📦',
        color: 'bg-orange-100 dark:bg-orange-950/60 text-orange-700 dark:text-orange-300',
    },
    inappropriate_behaviour: {
        label: 'Inappropriate Behaviour',
        emoji: '⚠️',
        color: 'bg-yellow-100 dark:bg-yellow-950/60 text-yellow-700 dark:text-yellow-300',
    },
    vandalism: { label: 'Vandalism', emoji: '🏚️', color: 'bg-red-100 dark:bg-red-950/60 text-red-700 dark:text-red-300' },
    late_end: { label: 'Late Finish', emoji: '⏰', color: 'bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300' },
    students_late: { label: 'Students Late', emoji: '🕐', color: 'bg-blue-100 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300' },
    improper_gear: { label: 'Improper Gear', emoji: '🎽', color: 'bg-purple-100 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300' },
    booking_timeout: {
        label: 'No-Show / Timeout',
        emoji: '🚫',
        color: 'bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300',
    },
    other: { label: 'Other', emoji: '📋', color: 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400' },
}
...
export default async function DefaulterStudents() {
    ...
    return (
        <div className="p-6 space-y-6">
            <header>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Defaulter Students</h1>
            ...
            <div className="flex items-center gap-4">
                <Card className="flex-1">
                    <CardContent className="p-4">
                        <div className="text-sm text-gray-600 dark:text-slate-400">Total Defaulters</div>
                        <div className="text-2xl font-bold text-red-600 dark:text-red-400">{defaulters.length}</div>
                    </CardContent>
                </Card>
                <Card className="flex-1">
                    <CardContent className="p-4">
                        <div className="text-sm text-gray-600 dark:text-slate-400">Active 14-Day Bans</div>
                        <div className="text-2xl font-bold text-orange-600 dark:text-orange-400">{activeBans}</div>
                    </CardContent>
                </Card>
                <Card className="flex-1">
                    <CardContent className="p-4">
                        <div className="text-sm text-gray-600 dark:text-slate-400">Lost Equipment Cases</div>
                                ...
                                {defaulters.map((student) => {
                                    ...
                                    return (
                                        <TableRow
                                            ...
                                            className={isBanned ? 'bg-red-50/40 dark:bg-red-950/20' : ''}
                                        >
                                            {/* Student info */}
                                            <TableCell>
                                                <div className="space-y-0.5">
                                                    ...
                                                    {student.student_phone && (
                                                        <a
                                                            ...
                                                            className="inline-flex items-center gap-1 text-xs text-[#004d40] dark:text-teal-400 hover:underline"
                                                        >
                                                    ...
                                                    className={`inline-flex items-center justify-center w-7 h-7 text-xs font-bold rounded-full ${
                                                        student.late_arrival_count >= 3
                                                            ...
                                                            : student.late_arrival_count >= 2
                                                              ...
                                                              : 'text-gray-600 dark:text-slate-300 bg-gray-100 dark:bg-slate-800'
                                                    }`}
                                                >
                                                    {student.late_arrival_count}/3
                                                </span>
                                            ...
                                            {/* Total violations */}
                                            <TableCell>
                                                <span className="inline-flex items-center justify-center w-7 h-7 text-xs font-bold text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-950/60 rounded-full">
                                            ...
                                            <TableCell className="whitespace-nowrap">
                                                {isBanned ? (
                                                    <div className="space-y-0.5">
                                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-red-100 dark:bg-red-950/60 text-red-700 dark:text-red-300">
                                            ...
                                            {/* Date */}
                                            <TableCell className="text-sm text-gray-600 dark:text-slate-400 whitespace-nowrap">
                                                {format(
